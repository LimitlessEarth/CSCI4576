...
int main (int argc, char** argv) {
    ...
    for (frame = 0; frame < num_iter; frame++) {
        ...
        #pragma omp parallel for schedule(static, REPLACE) private(dist, dx, dy, dz, a, ax, ay, az, i, j) shared(frame, Particles_a, Particles_b, dt)
        for (i = 0; i < num_part; i++) { // for particle i
            ...
            for (j = 0; j< num_part; j++) { // calculate force based on all other particles
                ...
                dist = sqrt(dx * dx + dy * dy + dz * dz) + REPLACE;
        ...
        printf("Iteration %d:\t%.REPLACE0f seconds\t%.REPLACE0f seconds\n", frame, end - start, end_writing - start_writing);
